...
@push('scripts')
{{-- Set route for prenatal records link --}}
<script>
    window.prenatalRecordIndexRoute = "{!! route('bhw.prenatalrecord.index') !!}";
</script>

{{-- Include shared BHW JavaScript utilities --}}
<script src="{{ asset('js/bhw/bhw.js') }}"></script>

{{-- Include patients index specific JavaScript --}}
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        @if(session('success'))
            showFlashMessages("{{ session('success') }}", null);
        @endif

        @if(session('error'))
            showFlashMessages(null, "{{ session('error') }}");
        @endif
    });
</script>
