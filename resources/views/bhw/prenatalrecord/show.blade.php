...
{{-- Include prenatal record show specific CSS --}}
<link rel="stylesheet" href="{{ asset('css/bhw/prenatalrecord-show.css') }}">
@endpush
...
<div class="space-y-2">
    <!-- Back Button -->
    <div class="mb-2">
        <a href="{{ route('bhw.prenatalrecord.index') }}" class="btn-action btn-view inline-flex items-center">
    ...
    <div class="bg-white rounded shadow-sm border">
        ...
        <div class="px-4 py-3">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
                ...
                <div class="bg-gray-50 rounded p-2 text-center">
                    <div class="text-xs text-gray-600 mb-1">
                        <i class="fas fa-calendar-check mr-1"></i>Last Visit
                    </div>
                    <p class="text-sm font-semibold text-gray-900">
                        @php
                            // Get the most recent completed checkup (past date)
                            $lastVisit = $prenatalRecord->patient->prenatalCheckups()
                                ->where('checkup_date', '<=', now())
                                ->orderBy('checkup_date', 'desc')
                                ->first();
                        @endphp
                        @if($lastVisit)
                            {{ $lastVisit->checkup_date->format('M d') }}
                        @else
                            None
                        @endif
                    </p>
                ...
                <div class="bg-gray-50 rounded p-2 text-center">
                    <div class="text-xs text-gray-600 mb-1">
                        <i class="fas fa-clock mr-1"></i>Next Visit
                    </div>
                    <p class="text-sm font-semibold text-gray-900">
                        @php
                            // Get the next upcoming checkup or scheduled visit (future date)
                            $nextVisit = $prenatalRecord->patient->prenatalCheckups()
                                ->where(function($query) {
                                    $query->where('checkup_date', '>', now())
                                          ->orWhere('next_visit_date', '>', now());
                                })
                                ->orderBy('checkup_date', 'asc')
                                ->first();
                        @endphp
                        @if($nextVisit)
                            @if($nextVisit->checkup_date > now())
                                {{ $nextVisit->checkup_date->format('M d') }}
                            @elseif($nextVisit->next_visit_date && \Carbon\Carbon::parse($nextVisit->next_visit_date) > now())
            ...
            <div class="flex justify-end space-x-1">
                <button onclick="openViewModal()" class="btn-action btn-view inline-flex items-center justify-center">
                    <i class="fas fa-eye mr-1"></i>
                    <span class="hidden sm:inline">View</span>
                </button>
                <button onclick="openEditModal()" class="btn-action btn-edit inline-flex items-center justify-center">
